...
image::survey1.png[AAP Job Template Editor for 'Linux/RHEL START Service on RHEL': Navigation tabs at top show Details, Access, Jobs, Schedules, Surveys, and Notifications. Click 'Surveys' tab to view and manage survey questions. Blue '+Create survey question' button in Surveys section starts workflow to add new survey question for user input collection when launching this job template,link=self,window=blank]
...
image::survey2.png[Survey Question Creation Form: (1) Question field - enter text 'Do you want to ENABLE the service (starts after a reboot)?' exactly as shown, (2) Answer variable name field - enter 'enable_service' with no spaces (this becomes the Ansible variable name), (3) Answer type dropdown - select 'Multiple Choice (single select)' from available options, (4) Scroll down to see additional fields for multiple choice options, (5) Blue 'Create survey question' button at bottom saves the question. After saving, this question appears in job template survey list and will prompt users for input when launching the job,link=self,window=blank]
...
image::jtsync.png[Self-Service Portal Sync Dialog: After clicking 'Sync now' button in top menu area, dialog appears asking 'What would you like to synchronize?' with checkboxes for different sync options including 'Organizations', 'Users', 'Teams', and 'Job Templates'. Select 'Job Templates' checkbox to synchronize only job template changes from AAP. Blue 'Ok' button at bottom starts synchronization process which takes a few seconds. Success message appears when sync completes,link=self,window=blank]
...
image::custom2.png[Custom Template Review Screen: Displays parsed YAML template details including: (1) Template name 'Manage RHEL Time Servers', (2) Metadata section showing template description and tags, (3) Parameters section listing input fields users will see (inventory selection, time server addresses, advanced options), (4) Steps section showing workflow actions including launching AAP job template 'RHEL / Update RHEL Time Servers'. Review all details for accuracy. Blue 'Import' button at bottom right adds this template to Portal catalog,link=self,window=blank]

** When you see `Step 4 Finish` the custom dynamic template will be imported successfully.

image::custom3.png[Import Success Screen: 'Step 4 Finish' heading with green checkmark icon. Success message states 'Template has been imported successfully' or similar confirmation. Template name 'Manage RHEL Time Servers' appears with imported status. Close or Done button returns to templates catalog where newly imported custom template now appears in template list,link=self,window=blank]
...
Log in to the link:{self_service_portal_url}[Self-Service Portal^] as `rheluser1` with password `{aap_admin_password}`. You should see the RHEL job templates and the new custom dynamic template you imported titled "Manage RHEL Time Servers"

image::custom4.png[Portal Template Catalog as rheluser1: Template tiles display showing mix of standard RHEL job templates (red tiles) like 'Linux/RHEL START Service', 'Linux/RHEL Package Management', alongside newly imported custom dynamic template 'Manage RHEL Time Servers' tile. Custom template tile may have distinctive styling or icon to indicate it is a dynamic template from Git repository rather than standard AAP job template. All tiles show blue Start button to launch,link=self,window=blank]
...
image::custom5.png[Custom Template Launch Wizard: Multi-step form for 'Manage RHEL Time Servers' showing: (1) Inventory selection dropdown to choose target RHEL systems, (2) Time server configuration fields for NTP/chrony server addresses, (3) 'Show Advanced Options?' checkbox which when enabled reveals additional parameters like timezone settings, service restart options, and validation steps. Form has Previous/Next buttons for navigation and blue Create button on final step to execute the custom template workflow which triggers the backing AAP job template with all selected parameters,link=self,window=blank]
