...
NOTE: **FOR BEST RESULTS... When logging in and out of Ansible Automation Platform and Self-Service portal as different users, please close and re-open your browser in private mode to ensure you are logged in / logged out as the correct user.**
...
image::survey1.png[Add survey question to the job template]

. Click on the `Surveys` tab.

.. Click on the `+Create survey question` button.

.. In the `Question` field type  `Do you want to ENABLE the service (starts after a reboot)?` in the `Question` field.

.. In the `Answer viriable name` field type `enable_service`.
...
image::survey2.png[Add survey question details]

.. You will now see the 3 survey question in the job template.

Modifications to the job template are automatically synchronized to the Self-Service portal.  This is done on a regular configurable interval in the HELM chart for the Self-Service portal. However, you can also trigger an instant synchronization of the job templates manually by clicking on the `Sync now` button while logged in as the `admin` user in the Self-Service portal.

. Log in to the `Self-Service portal` as `admin`. You wil see all the job templates.
...
image::jtsync.png[Self Service portal administrator user - sync now button]

. Log out of the `Self-Service portal` as `admin`.
...
image::survey3.png[Survey question added to the job template]
...
image::custom1.png[Select URL for custom dynamic template]
...
image::custom2.png[Review custom dynamic template]

.. When you see `Step 4 Finish` the custom dynamic template will be imported successfully.

image::custom3.png[Custom dynamic template imported successfully]
...
Log in to the `Self-Service portal` as `rheluser1`. You should see the RHEL job templates and the new custom dynamic template you imported. `⏰ Manage RHEL Time Servers`

image::custom4.png[The new custom dynamic template]
...
image::custom5.png[Launch the custom dynamic template from the Self-Service portal]

As you can see, custom dynamic templates provide a lot of flexibility to the user.

You have now completed this module. You have learned how to modify surveys on a job template and see the results in the Self-Service portal. You have also learned how to import a custom dynamic template and launch it from the Self-Service portal.

Thank you for attending this workshop! We hope you enjoyed it and learned a lot!