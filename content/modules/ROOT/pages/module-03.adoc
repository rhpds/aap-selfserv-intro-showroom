...
Log in to the link:{aap_controller_url}[Ansible Automation Platform,window=_blank] with the `{aap_admin_username}` user and password `{aap_admin_password}`.
...
Modifications to the job template are automatically synchronized to the Self-Service portal.  This is done on a regular configurable interval in the HELM chart for the Self-Service portal. However, you can also trigger an instant synchronization of the job templates manually by clicking on the `Sync now` button while logged in as the `{aap_admin_username}` user in the Self-Service portal.

* Log in to the link:{self_service_portal_url}[Self-Service Portal,window=_blank] as `{aap_admin_username}` with password `{aap_admin_password}`. You will see all the job templates.
...
* Log in to the link:{self_service_portal_url}[Self-Service Portal,window=_blank] as `rheluser1` with password `{aap_admin_password}`. You should see the RHEL job templates.
...
* Log in to the link:{self_service_portal_url}[Self-Service Portal,window=_blank] as `{aap_admin_username}` with password `{aap_admin_password}`.
...
Log in to the link:{self_service_portal_url}[Self-Service Portal,window=_blank] as `rheluser1` with password `{aap_admin_password}`. You should see the RHEL job templates and the new custom dynamic template you imported. `⏰ Manage RHEL Time Servers`
