...
At this point if you log in to the Self Service portal as any of the users you created, you will not be able to see any job templates.  But as an administrator you will be able to see all the job templates.

What you need to do is configure RBAC in Self Service portal to allow the Portal users to see the automation templates they are authorized to execute.

== Configuring RBAC in the Self Service Automation Portal

=== Log in to the `Self-Service portal` with the `admin` user, and the password provided for your AAP instance. When asked to `Authorize Self Service Portal?` click `Authorize`.
