...
image::createteam.png[Create Team dialog showing: Name field (text input for team name like 'cloud-team'), Organization dropdown (defaults to 'Default'), and blue 'Create team' button to confirm. Form expects team name to identify IT domain such as cloud, network, or rhel,link=self,window=blank]
...
image::assignroles.png[Roles tab for cloud-team showing list of currently assigned roles (initially empty) and blue 'Assign roles' button in top right to add new permissions. Click this button to start the multi-step role assignment workflow for job templates, inventories, and credentials,link=self,window=blank]
...
image::selectjobtemplates.png[Resource Selection step showing checkboxes next to multiple Cloud/AWS job templates including 'Cloud/AWS Create Instance', 'Cloud/AWS Check Region Connectivity', 'Cloud/AWS Terminate Instance'. Select all templates starting with 'Cloud/AWS' by clicking their checkboxes. Blue 'Next' button at bottom right proceeds to role selection,link=self,window=blank]

image::finish1strole.png[Review screen displaying summary of role assignment: Resource type shows 'Job Template', selected resources list all Cloud/AWS templates, and Role shows 'Job Template Execute' permission. Blue 'Finish' button at bottom right completes the assignment and returns to cloud-team Roles tab,link=self,window=blank]
...
image::assigninventoryrole.png[Inventory selection screen showing checkboxes for 'AWS Inventory', 'Azure Inventory', and 'GCP Inventory'. Select all three by clicking their checkboxes. This grants cloud-team Use permission to reference these inventories when launching job templates. Next button proceeds to role selection where you will choose 'Inventory Use',link=self,window=blank]
...
image::assigncredentialrole.png[Credential selection screen showing checkboxes for 'AWS Credential', 'Azure Credential', and 'RHEL - SSH Credentials'. Select all three to grant cloud-team Use permission for these credentials when running automation. Next button proceeds to role selection where you will choose 'Credential Use',link=self,window=blank]
...
image::assignuserrole.png[Users tab for cloud-team showing current team members list (initially empty) and blue 'Assign users' button in top right. Click button to open user selection dialog where you can select 'clouduser1' from available users and click 'Assign users' to add them to cloud-team,link=self,window=blank]
...
NOTE: Self-service portal has a default synchronization schedule of 60 minutes.
...
image::ssapadmin.png[Self-Service Portal home page as admin user: Top header shows 'Self-Service Portal' title and logged-in username. Left navigation includes Administration and Catalog menu items. Main content area displays job template tiles grouped by category - Cloud/AWS templates shown with blue styling, Network templates with green, RHEL templates with red. Each tile shows template name and blue 'Start' button. Administrator sees all approximately 15 templates. Non-admin users will see only templates authorized by RBAC policies,link=self,window=blank]
...
image::ssaprbac.png[Self-Service Portal Administration menu: Click 'Administration' in bottom left corner to expand menu showing options including 'RBAC', 'Plugins', and 'Settings'. Click 'RBAC' to navigate to RBAC policies page. On RBAC page, blue 'Create' button in top right area starts the policy creation workflow for new RBAC roles like ssa-portal-users and saa-portal-rhel-team,link=self,window=blank]
...
. For the `Add permission policies` field, from the `Select plugins` dropdown select `Catalog` and `Scaffolder`.
...
image::ssa-portal-users-role.png[RBAC role creation wizard for ssa-portal-users: Name field shows 'ssa-portal-users', Description shows 'Role for Cloud and Network teams'. Users and groups section displays 'cloud-team' and 'network-team' selected. Permission policies section shows Catalog plugin expanded with 'catalog.entity.read' checked, and Scaffolder plugin expanded with all permissions checked except 'scaffolder.template.management'. Blue Create button at bottom saves the policy,link=self,window=blank]
...
image::rolecondition.png[Conditional RBAC configuration interface for catalog.entity.read permission: Click checkmark icon in Actions column to open condition builder. Builder shows: (1) Condition type dropdown with 'Not' selected, (2) Rule dropdown with 'HAS_METADATA' selected, (3) Key field containing 'tags', (4) Value field containing 'custom'. Green checkmark appears when condition is valid. This configuration means rhel-team will see all templates EXCEPT those tagged with metadata key 'tags' and value 'custom', effectively hiding custom dynamic templates,link=self,window=blank]
image::saa-portal-rhel-team-role.png[RBAC role creation for saa-portal-rhel-team showing: Name field with 'saa-portal-rhel-team', Description 'Role for RHEL team', Users and groups showing 'rhel-team' selected. Catalog permissions show 'catalog.entity.read' with conditional rule icon indicating 'Not HAS_METADATA tags=custom' condition is applied. Scaffolder plugin shows all permissions checked except 'scaffolder.template.management'. This policy allows rhel-team to see RHEL templates but excludes custom tagged templates,link=self,window=blank]

The final view of this section should look like this:

image::final-rbac-roles.png[RBAC policies list showing two completed policies: 'ssa-portal-users' with description 'Role for Cloud and Network teams' assigned to 2 groups (cloud-team, network-team), and 'saa-portal-rhel-team' with description 'Role for RHEL team' assigned to 1 group (rhel-team) with conditional rule indicated. Both policies show catalog and scaffolder permissions configured. This view confirms successful RBAC configuration for all three teams,link=self,window=blank]

NOTE: Form more details on how to create RBAC roles and permissions, please refer to the link:https://docs.redhat.com/en/documentation/red_hat_ansible_automation_platform/latest/html/installing_self-service_automation_portal/self-service-accessing-deployment_aap-self-service-install#self-service-initial-rbac-setup_self-service-accessing-deployment[Self-Service Portal - RBAC Setup Documentation^].
